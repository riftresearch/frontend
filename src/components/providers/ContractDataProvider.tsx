...
export function ContractDataProvider({ children }: { children: ReactNode }) {
    ...
    const fetchSelectedAssetUserBalance = async () => {
        ...
        console.log('Refresh FetchBalance: Attempting...', {
        ...
        if (!currentAddress || !currentAsset || !currentProvider || !currentIsConnected) {
            console.log('FetchBalance: Aborted - Missing address, asset, provider, or not connected.');
        ...
        try {
            console.log(
                `FetchBalance: Fetching ${currentAsset.name} for ${currentAddress} on chain ${await currentProvider.getNetwork().then((n) => n.chainId)}`,
            );
            const balance = await getTokenBalance(currentProvider, currentAsset.tokenAddress, currentAddress, ERC20ABI);

            console.log(`FetchBalance: Raw balance for ${currentAsset.name}:`, balance.toString());
            ...
            console.log(`FetchBalance: Formatted balance for ${currentAsset.name}:`, formattedBalance);
        } catch (error) {
    ...
    useEffect(() => {
        const checkAndSwitchNetwork = async () => {
            ...
            if (!ethereum?.request || !selectedInputAsset || !isConnected) {
                console.log('SwitchNetwork: Conditions not met (no provider/request, asset, or not connected)');
                return;
            }
            ...
            try {
                ...
                if (currentChainId === contractChainID) {
                    console.log('SwitchNetwork: Already on correct network', currentChainId);
                    return;
                }

                console.log(
                    `SwitchNetwork: Mismatch. Current: ${currentChainId}, Target: ${contractChainID}. Attempting switch...`,
                );
                ...
                try {
                    ...
                    console.log('SwitchNetwork: Switched successfully via request.');
                } catch (switchError: any) {
                    console.error('SwitchNetwork: Error during switch', switchError);
                    if (switchError.code === 4902) {
                        console.log('SwitchNetwork: Chain not found (4902), attempting to add...');
                        try {
                            ...
                            console.log('SwitchNetwork: Added network successfully.');
                            await ethereum.request({
                                method: 'wallet_switchEthereumChain',
                                params: [{ chainId: hexChainId }],
                            });
                            console.log('SwitchNetwork: Switched successfully after adding.');
                        } catch (addError) {
    ...
    const fetchUserSwapsFromAddress = async () => {
        ...
        if (!currentAddress || !selectedInputAsset?.dataEngineUrl) {
            console.log('FetchSwaps: Aborted - Missing address or dataEngineUrl');
            setUserSwapsFromAddress([]);
            setUserSwapsLoadingState('error');
            return;
        }
        console.log(`FetchSwaps: Fetching for ${currentAddress}...`);
        try {
            ...
            console.log('FetchSwaps: Raw swaps received', rawSwaps);
            ...
            console.log('FetchSwaps: Typed swaps set in state', typedSwaps);
        } catch (error) {
    ...
    const refreshConnectedUserBalance = async () => {
        console.log('ContextRefresh: Manual refresh requested');
