...
import {
  FALLBACK_TOKEN_ICON,
  ETHEREUM_POPULAR_ADDRESSES,
  BASE_POPULAR_ADDRESSES,
} from "./constants";
...
function getPopularAddresses(network: Network): Set<string> {
  switch (network) {
    case Network.ETHEREUM:
      return ETHEREUM_POPULAR_ADDRESSES;
    case Network.BASE:
      return BASE_POPULAR_ADDRESSES;
    case Network.ALL:
...
export function searchTokens(network: Network, query: string, limit = 10): TokenData[] {
  ...
  const popularAddresses = getPopularAddresses(network);
  ...
  for (const r of ranked) {
    const isPopular = popularAddresses.has(r.item.addressLc);
  ...
  return ranked.slice(0, limit).map(({ item, chainId }) => ({
    name: item.name,
    ticker: item.ticker,
